...
// ─── GET /api/security-withdrawals ─────────────────────────────────────────────
// List the authenticated user's security withdrawals with optional filters.
//
// Query params:
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error('[GET /api/security-withdrawals]', err)
    return NextResponse.json<ApiResponse>(
      { success: false, message: 'Failed to fetch security withdrawals' },
      { status: 500 }
    )
  }
}

// ─── POST /api/security-withdrawals ────────────────────────────────────────────
// Create a new security withdrawal request.
//
...
export async function POST(req: NextRequest) {
  try {
    ...
    // TODO: Ideally verify user's security balance here. 
    // For now we just create the pending request for admin approval.
    ...
    return NextResponse.json<ApiResponse>(
      {
        success: true,
        message: 'Security withdrawal request submitted',
        data: txn,
      },
      { status: 201 }
    )
  } catch (err) {
    console.error('[POST /api/security-withdrawals]', err)
    return NextResponse.json<ApiResponse>(
      { success: false, message: 'Failed to create security withdrawal' },
      { status: 500 }
    )
  }
}
