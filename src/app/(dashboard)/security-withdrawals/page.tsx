...
async function fetchSecurityWithdrawals(filter: FilterState): Promise<PopulatedTransaction[]> {
  ...
  const res = await apiClient.get(`/security-withdrawals?${params}`)
...
export default function SecurityWithdrawalsPage() {
  ...
  const { data: txns = [], isLoading, isError } = useQuery<PopulatedTransaction[]>({
    queryKey:  ['security-withdrawals', filter],
    queryFn:   () => fetchSecurityWithdrawals(filter),
    staleTime: 20_000,
  })
  ...
  return (
    <div className="flex flex-col gap-4 animate-[fadeIn_200ms_ease-out]">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-lg font-bold text-primary">Security Withdrawals</h1>
        <Link
          href="/security-withdrawals/add"
          ...
          style={{
            ...
            boxShadow: '0 4px 12px var(--accent-amber-dim)',
          }}
        >
      ...
      <div className="page-card p-0! overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-3.5 border-b border-border-subtle">
          <p className="section-label mb-0">SECURITY WITHDRAWALS</p>
          {!isLoading && !isError && (
